...
async function dropDataBeforeSeed(database) {
  try {
    const entity = database.collection(collectionName);
    const result = await entity.drop();
    console.log(`documents deleted`);
  } catch (error) {
    console.error("Error drops database:", error);
  }
}
async function seedDatabase(database) {
  try {
    ...
    const entity = database.collection(collectionName);
    const result = await entity.insertMany(seed);
...
async function main() {
  ...
  await dropDataBeforeSeed(database);
